...
// ChromeOptions defines the Chrome-specific desired capabilities when using
...
type ChromeOptions struct {
	...
	MobileEmulation *ChromeMobileEmulation `json:"mobileEmulation,omitempty"`
	// PerfLoggingPrefs specifies options for performance logging.
	PerfLoggingPrefs *ChromePerfLoggingPreferences `json:"perfLoggingPrefs,omitempty"`
...
// ChromeMobileEmulation provides options for mobile emulation. Only
// DeviceName or both of DeviceMetrics and UserAgent may be set at once.
type ChromeMobileEmulation struct {
...
// ChromePerfLoggingPreferences specifies configuration options for performance
// logging.
type ChromePerfLoggingPreferences struct {
...
func (c ChromeOptions) AddExtension(path string) error {
	f, err := os.Open(path)
	if err != nil {
		return err
	}
	...
	if _, err := io.Copy(encoder, bufio.NewReader(f)); err != nil {
		return err
	}
	encoder.Close()
